...
class Ekf(object):
    ...
    def transition_update(self, u, dt):
        ...
        self.x = g
        self.Sigma = np.linalg.multi_dot([Gx, self.Sigma, np.transpose(Gx)]) + dt*np.linalg.multi_dot([Gu, self.R, np.transpose(Gu)])
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        ########## Code starts here ##########
        # TODO: Update self.x, self.Sigma.
        S = np.linalg.multi_dot([H, self.Sigma, np.transpose(H)]) + Q
        K = np.linalg.multi_dot([self.Sigma, np.transpose(H), np.linalg.inv(S)])
        self.x = self.x + np.dot(K,z)
        self.Sigma = self.Sigma - np.linalg.multi_dot([K,S,np.transpose(K)])
...
class EkfLocalization(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        xvec = self.x
        g, Gx, Gu = tb.compute_dynamics(xvec, u, dt)
        ########## Code ends here ##########
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(z_raw.shape[1]):
            ...
            for j in range(self.map_lines.shape[1]):
                vij = z_raw[:,i]-hs[:,j]
                vij[0] = angle_diff(z_raw[0,i],hs[0,j])
                Sij = np.linalg.multi_dot([Hs[j], self.Sigma, np.transpose(Hs[j])]) + Q_raw[i]
                dij = np.linalg.multi_dot([np.transpose(vij), np.linalg.inv(Sij), vij])
                if(dij < dist_min):
                    dist_min = dij
                    v_min = vij
                    Q_min = Q_raw[i]
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        xvec = self.x[0:3]
        g_temp, Gx_temp, Gu_temp = tb.compute_dynamics(xvec, u, dt)
        g[0:3] = g_temp
        Gx[0:3,0:3] = Gx_temp
        Gu[0:3,:] = Gu_temp
        ########## Code ends here ##########
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = np.array(v_list).reshape(1,-1).flatten()
        Q = scipy.linalg.block_diag(*Q_list)
        H = np.array(H_list).reshape(-1,33)
        ########## Code ends here ##########
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            x_cam_w, y_cam_w, _     = np.dot(np.array([[np.cos(theta), -np.sin(theta), x], [np.sin(theta), np.cos(theta), y], [0, 0, 1]]), np.array([x_cam, y_cam, 1]))
            if j >= 2:
